  ...
  if (fs.existsSync(homePath + AUX4_PACKAGE_DIRECTORY)) {
    fs.readdir(homePath + AUX4_PACKAGE_DIRECTORY, (err, files) => {
      files.forEach(file => {
        ...
        out.println(`- ${packageJson.package.name.yellow} [${packageJson.package.version.green}]`);
      });
    });
  }
}
...
async function install(args, parameters) {
  ...
  if (!aux4Json.package.author) {
    out.println(`There is no package author in the JSON file`.red);
    return;
  }
  ...
  const packageAuthor = aux4Json.package.author;
  ...
  out.println(
    `Package ${packageName.yellow} by ${packageAuthor.yellow} version ${packageVersion.yellow} has been installed successfully`
  );
}
